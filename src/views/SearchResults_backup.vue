          ...
          <div class="row maxWidth m-auto justify-content-center">
            <div class="row justify-content-center mb-2">
              <div class="card border-0 bg-transparent">
                <img v-if="images[0]"
                     :src="images[0]"
                     alt="" class="main-img w-auto shadowed-image pointerCursor" @click="showLightbox(0)">
              </div>
            </div>
            <div class="row mb-2">
              <div class="col-6 d-flex">
                <div class="card border-0 justify-content-center bg-transparent">
                  <img v-if="images[1]"
                       :src="images[1]"
                       alt="" class="secondary-img w-auto shadowed-image pointerCursor" @click="showLightbox(1)">
                </div>
              </div>
              <div class="col-6 d-flex">
                <div class="card border-0 justify-content-center bg-transparent">
                  <img v-if="images[2]"
                       :src="images[2]"
                       alt="" class="secondary-img w-auto shadowed-image pointerCursor" @click="showLightbox(2)">
                </div>
              </div>
              <div class="col-6 d-flex">
                <div class="card border-0 justify-content-center bg-transparent">
...
let sources = ref([]);
let images = ref([]);
let relatedQuestions = ref([]);
let llmResponse = ref("");
let positionClass = ref('fixed-bottom tempSearchPosition');
let graph = ref(null);
let answerBox = ref(null);
let searchServer = ref('http://127.0.0.1:8003');
...
const search_analysis_global = async (query) => {
  try {
    ...
    const url = `${searchServer.value}/search?query=${query}&pro_mode=${proMode.value}&date_context=${date_context}&stored_location=${storedLocation}`;
    ...
    eventSource.onmessage = async (event) => {
      ...
      switch (data.type) {
        ...
        case 'llm':
          ...
          if (data.text !== null && data.text !== undefined && data.text !== "" && !loadingUpdated) {
            isLoading.value = false;
            loadingUpdated = true;
          }
          break;
...
const updateSearchResults = (data) => {
  ...
  if (data.graph) {
    ...
    if (data.images && data.images.length > 0) {
      graph.value.imageUrl = data.images[0].imageUrl;
    }
  }
...
const updateFavicons = () => {
  if (sources.value.length > 0) {
    sources.value.forEach(async (source) => {
      let url = new URL(source.link);
...
const searchNew = async (question) => {
  searchTitle.value = question;
  await search_analysis_global(question);
}
